...
public class Updater {
	...
	public enum UpdateResult {
		...
		private UpdateResult(final int value) {
			this.value = value;
		}
		...
		public static Updater.UpdateResult getResult(final int value) {
			return valueList.get(value);
		}

		static {
			for (final Updater.UpdateResult result : Updater.UpdateResult
					.values()) {
	...
	public enum UpdateType {
		...
		public static Updater.UpdateType getResult(final int value) {
			return valueList.get(value);
		}

		static {
			for (final Updater.UpdateType result : Updater.UpdateType.values()) {
				valueList.put(result.value, result);
			}
		}
	}
	 ...
	 *            updates in console
	 */
	public Updater(final Plugin plugin, final String slug, final File file,
		...
		if (url != null) {
			...
			if (versionCheck(versionTitle)) {
				final String fileLink = getFile(versionLink);
	...
	private void saveFile(final File folder, final String file, final String u) {
		...
		try {
			...
			for (final File xFile : new File("plugins/" + updateFolder)
					.listFiles()) {
			...
			for (final File dFile : new File(zipPath).listFiles()) {
				if (dFile.isDirectory()) {
					if (pluginFile(dFile.getName())) {
						final File oFile = new File("plugins/"
	...
	private boolean hasTag(final String version) {
		for (final String string : noUpdateTag) {
